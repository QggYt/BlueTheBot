...
const CATEGORY_PAGE_PREFIX = 'help-category-page';
...
function getCategoryIcon(category) { return CATEGORY_ICONS[formatCategoryName(category)] || '🔹'; }
...
function buildHelpEntries(command) {
    ...
    const entries = [];
    const pushSubcommand = (name, description, prefix = data.name) => entries.push({ baseName: data.name, displayName: `${prefix} ${name}`, description: description || data.description || 'No description', category });
    for (const option of data.options || []) {
        if (option.type === SUBCOMMAND_TYPE) pushSubcommand(option.name, option.description);
        else if (option.type === SUBCOMMAND_GROUP_TYPE) for (const nested of option.options || []) if (nested?.type === SUBCOMMAND_TYPE) pushSubcommand(nested.name, nested.description || option.description, `${data.name} ${option.name}`);
    }
    if (entries.length === 0) entries.push({ baseName: data.name, displayName: data.name, description: data.description || 'No description', category });
    return entries;
}
function getActiveHelpEntries(client) {
    const entries = [];
    const seen = new Set();
    for (const command of client?.commands?.values?.() || []) for (const entry of buildHelpEntries(command)) {
...
async function fetchRegisteredCommands(client) {
    ...
    try {
        for (const command of await client.application.commands.fetch()) registeredCommands.set(command[1]?.name || command.name, command[1] || command);
        for (const guild of client.guilds?.cache?.values?.() || []) {
            try { for (const command of (await guild.commands.fetch()).values()) registeredCommands.set(command.name, command); }
            catch (error) { logger.debug(`Could not fetch guild commands for ${guild.id}:`, error?.message); }
        }
    } catch (error) { logger.debug('Could not fetch registered commands for Help:', error?.message); }
...
function makeCategoryPageId(_category, action) { return `${CATEGORY_PAGE_PREFIX}_${action}`; }
...
export async function createCategoryCommandsMenu(category, page = 1, client) {
    ...
    const entries = getActiveHelpEntries(client).filter((entry) => entry.category === requestedCategory).sort((a, b) => a.displayName.localeCompare(b.displayName));
    ...
    const embed = createEmbed({ title: `${getCategoryIcon(requestedCategory)} ${requestedCategory} Commands`, description: entries.length ? `Showing ${entries.length} active command entries for this category.` : 'No active commands found in this category.' });
...
export async function createAllCommandsMenu(page = 1, client) {
    ...
    const embed = createEmbed({ title: '📋 All Commands', description: `Showing ${allCommands.length} active command entries. Duplicate command entries are filtered before display.` });
    addServerStats(embed, client);
...
export const helpCategorySelectMenu = {
    ...
    async execute(interaction, client) {
        try {
            ...
            const result = selectedCategory === ALL_COMMANDS_ID ? await createAllCommandsMenu(1, client) : await createCategoryCommandsMenu(selectedCategory, 1, client);
